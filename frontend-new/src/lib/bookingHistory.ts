...
export const HISTORY_FIELDS: Record<string, FieldDef> = {
  ...
  guest_title:      { label: "Titel",      format: (v) => String(v || "–") },
  guest_first_name: { label: "Vorname",    format: (v) => String(v || "–") },
  guest_last_name:  { label: "Nachname",   format: (v) => String(v || "–") },
  phone:          { label: "Mobil",        format: (v) => String(v || "–") },
  landline:       { label: "Festnetz",     format: (v) => String(v || "–") },
  ...
  cancellationFee: { label: "Stornogebühren", format: (v) => `${Number(v).toLocaleString("de-DE")} €` },
